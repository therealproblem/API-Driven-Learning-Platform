...
import { createUser, getUserByEmail } from "../db/helpers/usersHelper.ts";
...
export const register = async (req: Request, res: Response) => {
    ...
    if (emailExist) 
        return res.status(409).json({ error: 'Email already exist' }); // <-- return here

    const newUser = await createUser(req.body.email, hash(req.body.password), req.body.name);

    if (newUser === null) 
        return res.status(500).json({ error: 'DB Error: unable to insert' }); // <-- return here
